...
struct MANGOS_DLL_DECL npc_twilight_volunteerAI : public ScriptedAI
{
    ...
    void Sacriface(uint8 phase)
    {
        ...
        switch(m_uiPhase)
        {
            case SACRIFACE_CHOOSEN:
                switch(urand(0, 1))
                {
                    case 0: DoScriptText(SAY_VOLUNTEER_1, m_creature);
                        break;
                    case 1: DoScriptText(SAY_VOLUNTEER_2, m_creature);
                        break;
                }
    ...
    {
        if (!m_pInstance || m_bIsDead)
            return;

        if (m_pInstance->GetData(TYPE_JEDOGA) != IN_PROGRESS)
